...
class speciality_form(forms.Form):
	...
	spec = forms.ChoiceField(choices = [(r.id, r.human_name) for r in Specialization.objects.order_by('name')], label='Specialty.', required = True)
...
class advanced_form(forms.Form):
	...
	spec_adv = forms.ChoiceField(choices = [(r.id, r.human_name) for r in Specialization.objects.order_by('name')], label='Specialty.', required = True)
