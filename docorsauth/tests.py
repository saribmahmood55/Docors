...
from .forms import docorsUserChangeForm, docorsUserCreationForm
...
class docorsUserCreationFormTest(TestCase):

    def setUp(self):
        get_user_model().objects.create_user('testclient@example.com', 'Test Client', 'test123')
    ...
    def test_user_already_exists(self):
        ...
        form = docorsUserCreationForm(data)
    ...
    def test_both_passwords(self):
        ...
        data = {'email': 'testclient@example.com', 'full_name':'Test Client'}
        form = docorsUserCreationForm(data)
...
@override_settings(USE_TZ=False, PASSWORD_HASHERS=('django.contrib.auth.hashers.SHA1PasswordHasher',))
class docorsUserChangeFormTest(TestCase):
    ...
    def test_username_validity(self):
        ...
        form = docorsUserChangeForm(data, instance=user)
        self.assertFalse(form.is_valid())
        self.assertEqual(form['email'].errors,
                         [_('Enter a valid email address.')])
    ...
    def test_bug_14242(self):
        ...
        # docorsUserChangeForm.

        class MyUserForm(docorsUserChangeForm):
            def __init__(self, *args, **kwargs):
                super(MyUserForm, self).__init__(*args, **kwargs)
                self.fields['groups'].help_text = 'These groups give users different permissions'

            class Meta(docorsUserChangeForm.Meta):
                fields = ('groups',)
    ...
    def test_unsuable_password(self):
        ...
        user.save()
        form = docorsUserChangeForm(instance=user)
    ...
    def test_bug_19133(self):
        ...
        user = get_user_model().objects.get(email='testclient@example.com')
        form_for_data = docorsUserChangeForm(instance=user)
        ...
        form = docorsUserChangeForm(instance=user, data=post_data)
...
"""
class docorsUserAdminTest(TestCase):
    ...
    def setUp(self):
        ...
        if settings.AUTH_USER_MODEL == "custom_user.docorsUser":
            self.app_name = "custom_user"
            self.model_name = "docorsUser"
        ...
        elif settings.AUTH_USER_MODEL == "test_custom_user_subclass.MyCustomdocorsUser":
            self.app_name = "test_custom_user_subclass"
            self.model_name = "mycustomdocorsUser"
            self.model_verbose_name = "MyCustomdocorsUserVerboseName"
            self.model_verbose_name_plural = "MyCustomdocorsUserVerboseNamePlural"
            if django.VERSION[:2] < (1, 7):
