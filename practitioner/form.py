...
class PractitionerForm(forms.Form):
	...
	credentials = forms.CharField(max_length=500, required=True, label="Credentials.", widget=forms.TextInput(attrs={'data-provide':'typeahead', 'class':'input-huge', 'required':'true', 'class':'form-control'}))
	achievements = forms.CharField(max_length=800, required=False, label='Achievements.', widget=forms.Textarea(attrs={'placeholder': 'education, training & achievements','rows':'3', 'class':'form-control'}))
	...
	message = forms.CharField(max_length=160, required=False, label='Short message for patients.', widget=forms.TextInput(attrs={'placeholder': 'available, out of country till..', 'class':'form-control'}))
	...
	#Practice Location
	practice_name = forms.CharField(max_length=50, required=True, label='Name.', widget=forms.TextInput(attrs={'data-error':'Please provide a valid name','placeholder': 'Adil Hospital', 'required':'true', 'class':'form-control'}))
	address = forms.CharField(max_length=500, required=True, label='Street address.', widget=forms.Textarea(attrs={'data-error':'Please provide a valid address','placeholder': '82-XX, DHA','rows':'1', 'class':'form-control','required':'true'}))
	...
	contact_number = forms.CharField(max_length=100, label="Contact number(s)",widget=forms.TextInput(attrs={'data-error':'Contact number required','placeholder': '0423585xxxx, 03214567xxx', 'required':'true', 'class':'form-control'}), required=True)
	checkup_fee = forms.IntegerField(widget=forms.TextInput(attrs={'placeholder': '1500', 'class':'form-control'}), label="Checkup fee (pkr)." )
