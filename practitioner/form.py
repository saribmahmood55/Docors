...
class PractitionerForm(forms.Form):
	...
	#practitioner	
	practitioner_title = forms.ChoiceField(choices= TITLE, label="Title", initial='1', required=True)
	...
	credentials = forms.CharField(max_length=500, required=True, label="Credentials.", widget=forms.TextInput(attrs={'data-provide':'typeahead'}))
	achievements = forms.CharField(max_length=800, required=False, label='Achievements.', widget=forms.Textarea(attrs={'placeholder': 'education, training & achievements','rows':'3'}))
	experience = forms.IntegerField(label='Experience (years).', initial='0', required=True)
	...
	#Practice Location
	practice_name = forms.CharField(max_length=50, required=True, label='Name.', widget=forms.TextInput(attrs={'placeholder': 'Adil Hospital'}))
	address = forms.CharField(max_length=500, required=True, label='Street address.', widget=forms.Textarea(attrs={'placeholder': '82-XX, DHA','rows':'1'}))
	...
	contact_number = forms.CharField(max_length=100, label="Contact number(s)",widget=forms.TextInput(attrs={'placeholder': '0423585xxxx, 03214567xxx'}), required=True)
	checkup_fee = forms.IntegerField(widget=forms.TextInput(attrs={'placeholder': '1500'}), label="Checkup fee (pkr)." )
	services = forms.CharField(max_length=800, widget=forms.Textarea(attrs={'rows':'3'}), label="Services offered", required=False)
